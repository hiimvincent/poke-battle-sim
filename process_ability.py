...
def selection_abilities(poke: pk.Pokemon, battlefield: bf.Battlefield, battle: bt.Battle):
    if poke.has_ability('drizzle') and battlefield.weather != gs.RAIN:
        ...
        battle.add_text(poke.nickname + ' copied ' + poke.enemy.current_poke.nickname + '\'s ' + poke.enemy.current_poke.ability + '!')
        ...
        battle.add_text(poke.nickname + ' frisked ' + poke.enemy.current_poke.nickname + ' and found its ' + poke.enemy.current_poke.item + '!')
...
def on_hit_abilities(attacker: pk.Pokemon, defender: pk.Pokemon, battle: bt.Battle, move_data: Move) -> bool:
    ...
    if defender.has_ability('static') and made_contact and randrange(10) < 3:
        ...
        battle.add_text(defender.nickname + ' transformed into the ' + move_data.type.upper() + ' type!')
        ...
        pm.burn(attacker, battle)
            ...
            and not 'poison' in attacker.types and randrange(10) < 3:
...
def homc_abilities(attacker: pk.Pokemon, defender: pk.Pokemon, battlefield: bf.Battlefield, battle: bt.Battle, move_data: Move) -> float:
    ability_mult = 1
